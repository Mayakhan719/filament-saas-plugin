...
namespace Maya\FilamentSaasPlugin\Livewire;
...
use Maya\FilamentSaasPlugin\Models\PaymentGateway;
...
class PaymentProcess extends Component implements HasForms, HasActions
{
    ...
    {
        return $form->schema([
            Section::make('Choose Payment Method')
                ->schema([
                    Radio::make('gateway')
    ...
    public function process()
    {
        ...
        $drivers = config('filament-saas-plugin.drivers');
        $new = false;
        ...
        if (!$new) {
            $new = "Maya\FilamentSaasPlugin\\Services\\Drivers\\{$dirName}";
        }
    ...
    {
        $viewName = $this->viewToRender ?: 'filament-saas-plugin::livewire.payment-process';

        return view($viewName, [
            'payment' => $this->payment,
            'data' => $this->data,
        ])
            ->extends('filament-saas-plugin::filament.client.layouts.payment')
            ->section('content');
    }
}
