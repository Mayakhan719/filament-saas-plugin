...
namespace Maya\FilamentSaasPlugin\Services\Drivers;

use Maya\FilamentSaasPlugin\Facades\FilamentSubscriptions;
use Maya\FilamentSaasPlugin\Models\Account;
...
abstract class Driver
{
    ...
    public static function subscription($payment_log)
    {
        ...
        if ($eventClass === \Maya\FilamentSaasPlugin\Events\SubscribePlan::class) {
            $tenant->newPlanSubscription($plan->slug, $plan);
            Event::dispatch(new $eventClass([    // event Maya\FilamentSaasPlugin\Events\SubscribePlan
                "old" => null,
                "new" => $plan,
        ...
        if ($eventClass === \Maya\FilamentSaasPlugin\Events\RenewPlan::class) {
        ...
        if ($eventClass === \Maya\FilamentSaasPlugin\Events\ChangePlan::class) {
            $currentSubscription = $tenant->planSubscriptions()->first();
            Event::dispatch(new $eventClass([
                "old" => $currentSubscription->plan,
                "new" => $plan,
